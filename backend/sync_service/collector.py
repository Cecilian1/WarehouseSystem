...
def _reconcile_state(local_db_path: str, state: dict[str, int]) -> dict[str, int]:
    ...
    try:
        for table in INCREMENTAL_TABLES:
            ...
            # 电脑端写入使用负数 ID，开发板增量接口的 after 游标只接受
            # 非负数。仅用开发板 ID 空间校准游标，避免发送 after=-1 导致 422。
            board_max = max(0, local_max)
            if board_max < reconciled[table]:
                reconciled[table] = board_max
    finally:
        conn.close()
    return reconciled
