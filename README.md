...
| `QWEN_API_KEY` | 每日智能报告调用通义千问时必填。只放在根目录 `.env` 或服务器环境变量中，禁止写入 `wechat-miniprogram/`；未配置时自动生成本地兜底报告 |
| `QWEN_BASE_URL` / `QWEN_MODEL` | 通义千问 OpenAI 兼容接口地址与模型名，默认使用 `qwen-plus` |
...
- 小程序 5 个主 Tab 页右下角均有“芯鲜 AI 助手”悬浮入口；点击“AI 每日报告”会汇总当天库存、新鲜度、温湿度、出入库和预警数据。
- 后端接口为 `GET /api/reports/daily`，同一天默认复用缓存；传入 `refresh=true` 可重新生成。
- 模型调用只发生在 `backend/api_service/daily_report.py`。`QWEN_API_KEY` / `DASHSCOPE_API_KEY` 只从后端进程环境读取，不会出现在小程序源码或接口响应中。
- 通义千问不可用、超时或输出格式异常时，接口会返回同一份本地数据生成的规则报告，并通过 `source` 与 `fallbackReason` 告知前端。
- 修改 `.env` 中的模型配置后需要重启后端；不需要重新执行小程序配置同步脚本。
