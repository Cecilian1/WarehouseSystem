...
`GET /api/reports/daily` 汇总 `produce_info`、`stock_summary`、`inventory_log`、
`env_log` 与 `alert_record` 中的当天数据，并由后端调用通义千问组织中文报告。

- 查询参数：`refresh=true` 时忽略当日内存缓存并重新生成。
- 返回的 `data` 同时包含权威数据快照和报告文案；`riskLevel` 始终由本地规则计算，模型不能修改风险等级或原始数值。
- `source=qwen` 表示通义千问生成成功；`source=local-fallback` 表示使用本地规则文案，具体原因见 `fallbackReason`。
- 认证密钥只允许通过后端环境变量 `QWEN_API_KEY`（或 `DASHSCOPE_API_KEY`）提供，禁止由小程序传入或返回给客户端。
