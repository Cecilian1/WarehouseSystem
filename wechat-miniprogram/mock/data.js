...
function dailyReport() {
  const warningItems = inventory.filter((item) => ['mild', 'expiring'].includes(item.freshness))
  const spoiledItems = inventory.filter((item) => item.freshness === 'spoiled')
  const totalQuantity = inventory.reduce((sum, item) => sum + item.quantity, 0)
  const weightedFreshness = inventory.reduce(
    (sum, item) => sum + item.freshnessScore * item.quantity,
    0
  )
  return {
    title: '2026-07-23 每日仓储报告',
    summary: '今日温湿度整体适宜，但有多份叶菜和水果需要优先处理，建议按新鲜度顺序安排食用。',
    riskLevel: 'attention',
    generatedAt: '2026-07-23T20:30:00',
    source: 'qwen',
    sourceLabel: '通义千问 · qwen-plus',
    fallbackReason: '',
    cached: false,
    environment: {
      valid: true,
    ...
    inventory: {
      produceTypes: inventory.length,
      totalQuantity,
      todayInbound: 7,
      todayOutbound: 3,
      todayRecognitionCount: 12
    },
    freshness: {
      averageScore: Math.round(weightedFreshness / totalQuantity),
      fresh: inventory.filter((item) => item.freshness === 'fresh').reduce((sum, item) => sum + item.quantity, 0),
      warning: warningItems.reduce((sum, item) => sum + item.quantity, 0),
      spoiled: spoiledItems.reduce((sum, item) => sum + item.quantity, 0),
      riskyItems: warningItems.concat(spoiledItems).map((item) => ({
        name: item.name,
        quantity: item.quantity,
        unit: item.unit,
        status: item.freshness === 'spoiled' ? '腐败' : '临期',
        freshnessScore: item.freshnessScore,
        remainingDays: item.remainingDays
      })).sort((a, b) => a.freshnessScore - b.freshnessScore).slice(0, 6)
    },
    alerts: { pending: 6, critical: 2, warning: 4, recent: [] },
    highlights: [
      '当前温度 3.6°C、湿度 88%RH，整体处于适宜储存区间。',
      `冰箱共有 ${inventory.length} 类果蔬、合计 ${totalQuantity} 件。`,
      `综合新鲜度为 ${Math.round(weightedFreshness / totalQuantity)} 分，临期或轻度老化果蔬共 ${warningItems.length} 类。`,
      '草莓、菠菜和生菜的处理优先级较高。'
    ],
    recommendations: [
      '立即检查并移出生菜，避免与其他叶菜交叉接触。',
      '今天优先食用菠菜和草莓，并复查西红柿表皮状态。',
      '继续保持当前冷藏环境，减少频繁开门造成的短时升温。',
      '在预警中心确认仍未处理的紧急事项。'
    ]
  }
}
...
function aiAssistant(data = {}) {
  const type = data.type || 'freshness'
  const common = {
    type,
    question: data.question || '',
    urgency: type === 'alerts' ? 'high' : 'attention',
    source: 'qwen',
    sourceLabel: '通义千问 · qwen-plus',
    grounded: true,
    fallbackReason: '',
  ...
  if (type === 'environment') {
    return {
      ...common,
      title: '温湿度诊断',
      summary: '当前温度3.6°C、湿度88%RH，整体适宜，今日有少量短时波动。',
      bullets: [],
      format: 'paragraph'
    }
  }
  if (type === 'alerts') {
    return {
      ...common,
      title: '风险预警',
      summary: '当前最紧急的是腐败与临期果蔬，其次是尚未确认的环境预警。',
      bullets: ['立即移出生菜并检查相邻叶菜。', '今天优先食用菠菜和草莓。', '确认温度预警并检查冰箱门。'],
      format: 'list'
    }
  }
  return {
    ...common,
    title: '新鲜度分析',
    summary: '整体新鲜度尚可，但生菜、菠菜和草莓需要优先处理。',
    bullets: ['立即移出疑似腐败的生菜。', '今天优先食用菠菜和草莓。', '西红柿建议两天内食用。'],
    format: 'list'
  }
}
...
const routes = {
  ...
  'GET /reports/daily': dailyReport,
  'POST /assistant/analyze': aiAssistant,
  'GET /records': (query = {}) => {
