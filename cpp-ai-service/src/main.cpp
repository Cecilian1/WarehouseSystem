...
public:
    ...
    std::vector<Detection> detect(const cv::Mat &image, double &preprocess_ms,
        ...
        std::cerr << "AI阶段: 提交YOLO输入" << std::endl;
        if (extractor.input("in0", input) != 0) throw std::runtime_error("YOLO NCNN输入失败");
        ncnn::Mat ncnn_output;
        std::cerr << "AI阶段: 开始YOLO推理" << std::endl;
        if (extractor.extract("out0", ncnn_output) != 0) throw std::runtime_error("YOLO NCNN推理失败");
        std::cerr << "AI阶段: YOLO推理完成" << std::endl;
        forward_ms = elapsed_ms(forward_started);

        const auto postprocess_started = Clock::now();
        const int output_rows = ncnn_output.h;
        const int output_columns = ncnn_output.w;
        if ((ncnn_output.dims != 2 && ncnn_output.dims != 3) ||
            output_rows <= 0 || output_columns <= 0 || ncnn_output.c > 1) {
            throw std::runtime_error(
                "YOLO NCNN输出形状不正确: dims=" + std::to_string(ncnn_output.dims) +
                ", w=" + std::to_string(ncnn_output.w) +
                ", h=" + std::to_string(ncnn_output.h) +
                ", c=" + std::to_string(ncnn_output.c));
        }
        cv::Mat output(output_rows, output_columns, CV_32F);
        if (ncnn_output.dims == 2) {
            for (int row = 0; row < output_rows; ++row) {
                std::copy(ncnn_output.row(row), ncnn_output.row(row) + output_columns,
                          output.ptr<float>(row));
            }
        } else {
            const ncnn::Mat output_channel = ncnn_output.channel(0);
            for (int row = 0; row < output_rows; ++row) {
                std::copy(output_channel.row(row), output_channel.row(row) + output_columns,
                          output.ptr<float>(row));
            }
        }
    ...
    try {
        ...
        while (true) {
            ...
            try {
                ...
                std::cerr << "AI阶段: 图片读取完成 " << image.cols << "x" << image.rows << std::endl;
